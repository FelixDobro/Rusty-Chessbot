...
impl Board {
    ...
    pub fn calculate_mg(&self) -> i16 {
        ...
        let mut pawn_bb_white = self.piece_bb[Piece::Pawn.index() + WhiteSide::OFFSET];
        while pawn_bb_white != EMPTY_BB {
            val += PAWN_MG[WhiteSide::INDEX][pawn_bb_white.lsb().index()];
            pawn_bb_white.pop_lsb();
        }   


        let mut pawn_bb_black = self.piece_bb[Piece::Pawn.index() + BlackSide::OFFSET]; 
        while pawn_bb_black != EMPTY_BB {
            val -= PAWN_MG[BlackSide::INDEX][pawn_bb_black.lsb().index()];
            pawn_bb_black.pop_lsb();
        }


        let mut bishop_bb_white = self.piece_bb[Piece::Bishop.index() + WhiteSide::OFFSET]; 
        while bishop_bb_white != EMPTY_BB {
            val -= PAWN_MG[BlackSide::INDEX][bishop_bb_white.lsb().index()];
            bishop_bb_white.pop_lsb();
        }
...
const PHASE_VALUES: [i16; 7] = [0, 1, 1, 2, 4, 0, 0];
...
pub static PAWN_MG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
    ...
    let mut i = 0;
    let mut black = [0i16; 64];
    while i < 64 {
        black[i] = white[flip(i)];
        i += 1;
    }
    Box::new([white, black])
});

pub static PANW_EG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static KNIGHT_MG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static KNIGHT_EG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static BISHOP_MG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static BISHOP_EG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static ROOK_MG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static ROOK_EG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static QUEEN_MG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static QUEEN_EG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static KING_MG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
...
pub static KING_EG: LazyLock<Box<[[i16; 64]; 2]>> = LazyLock::new(|| {
    let white = [
