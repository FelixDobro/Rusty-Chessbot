...
use std::ops::{BitAnd, BitAndAssign, BitOr, BitOrAssign, BitXor, BitXorAssign, Shl,ShlAssign, Shr, ShrAssign, Not};
...
pub const WHITE_KING_CASTLE_BLOCKERS: Bitboard = Bitboard((1u64 << Square::F1.u8()) | (1u64 << Square::G1.u8())); 
pub const BLACK_KING_CASTLE_BLOCKERS: Bitboard = Bitboard((1u64 << Square::F8.u8()) | (1u64 << Square::G8.u8()));
pub const WHITE_QUEEN_CASTLE_BLOCKERS: Bitboard = Bitboard((1u64 << Square::B1.u8()) | (1u64 << Square::C1.u8()) | (1u64 << Square::D1.u8()));
pub const BLACK_QUEEN_CASTLE_BLOCKERS: Bitboard = Bitboard((1u64 << Square::B8.u8()) | (1u64 << Square::C8.u8()) | (1u64 << Square::D8.u8()));
...
pub const EN_PESSANT_UPDATES: [Bitboard; 64] = {
    let mut table: [Bitboard; 64] = [EMPTY; 64];
    let mut square = 0;
    while square < 64 {
...
pub const EN_PASSANT_RM_SQUARES: [Bitboard; 64] = {
    let mut table: [Bitboard; 64] = [EMPTY; 64];
...
pub const STRAIGHT_LINES: [Bitboard; 64] = {
    let mut table: [Bitboard; 64] = [EMPTY; 64];
    let mut sq = 0;
    ...
    while sq < 64 {
        ...
        let mut slider = 0;
        while slider < 8 {
            vertical_board |= ((vertical_board << 8) | (vertical_board >> 8)) & !(RANK_1.u64() | RANK_8.u64());
            horizontal_board |=
                ((horizontal_board << 1) | (horizontal_board >> 1)) & !(FILE_A.u64() | FILE_H.u64());
            slider += 1;
        }
...
pub const DIAGONAL_LINES: [Bitboard; 64] = {
    let mut table: [Bitboard; 64] = [EMPTY; 64];
    let mut sq = 0;
    ...
    while sq < 64 {
        ...
        while slider < 6 {
            ...
            moves |=
                ((left_prefix >> slider * 8) & !RANK_1.u64()) | ((left_prefix << 8 * slider) & !(RANK_8.u64()));
...
pub static STRAIGHT_LINES_MAGIC: LazyLock<Box<[[Bitboard; 4096]; 64]>> = LazyLock::new(|| {
    let mut table: Box<[[Bitboard; 4096]; 64]> = Box::new([[EMPTY; 4096]; 64]);
...
pub const KING_PATTERNS: [Bitboard; 64] = {
...
pub const KNIGHT_PATTERNS: [Bitboard; 64] = {
...
const fn pre_calculate_knight_moves(sq: u8) -> Bitboard {
    ...
    let un_bit_board =((prefix >> 2) & !(FILE_G.u64() | FILE_H.u64())) | ((prefix << 2) & !(FILE_A.u64() | FILE_B.u64())) | (first_half);
